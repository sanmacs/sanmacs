...
<%
	...
	rs=stmt.executeQuery("Select msid,sender,rece_date,message from message where receiver='"+myeid+"' and category='INBOX' order by rece_date desc");
...
<%
	...
	//System.out.println("Satish:"+buf.capacity());

		//if (buf.capacity()>=102400) {
		if (buf.capacity()>=1024) {
		out.println("<tr><td colspan=3><p id=ftp align=justify><b>You are running out of memory.</b> Some of your messages are waiting in suspense account. Please delete some messages to create space for waiting messages.</p></td></tr>");
		}
		else {
			rs1=stmt.executeQuery("Select * from suspense_account where receiver='"+myeid+"'");
			...
			rcount=rs1.getRow();
				...
				if (rcount==1) {
				out.println("<tr><td colspan=3><p id=ftp align=justify><b>You are running out of memory.</b> One of your message is waiting in suspense account. Please delete some messages to create space for waiting messages.</p></td></tr>");
				}
				else if(rcount > 1) {
				out.println("<tr><td colspan=3><p id=ftp align=justify><b>You are running out of memory.</b> Some of your messages are waiting in suspense account. Please delete some messages to create space for waiting messages.</p></td></tr>");
				}
...
<tr><th colspan=3><p id="ftp" align="center" style="color:blue">Inbox Empty!</p></th></tr>
<%	
	}
%>
</table>
...
<tr><th><p id=ftp align=left>Note:</p></th></tr>
<tr><td>
<ul>
<li><p id=ftp align=justify>If your messages goes out of memory, then only five of your extra messages will be saved in suspense account. Rest all will be discarded.</li>
<li><p id=ftp align=justify>Suspense account messages will be transfered to your main account if and only if enough space will be available for messages.</li>
<li><p id=ftp align=justify>To create space, delete some of your unnecessary messages from <b>INBOX / OUTBOX</b>.</li>
</td></tr>
</table>
</form>
</DIV>
...
<tr><td width=177 height=<%=256+(newcount*33) %> bgcolor=#933900></td></tr>
</table>
</DIV>

<DIV class="Object9" style="top:<%=720+(newcount*33) %>px">
...
<DIV class="Object10" style="top:<%=750+(newcount*33) %>px">
