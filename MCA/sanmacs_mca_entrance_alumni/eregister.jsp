...
<%
	...
	gen_code=request.getParameter("gcode");
...
%>
	...
	<tr><td height=10></td></tr>
	<tr><th><p id="ftp" align=justify>Information given by you is incorrect. Your information doesn't Exists!</p></th></tr>
	<tr><td height=20></td></tr>
	<tr><th><input class=a type="button" onclick="history.back();" value="Back"></th></tr>
<%
	}
	else {
	rs2=st.executeQuery("Select alumnimeet from Alumni where Email='"+Email+"'");
	rs2.next();
	meetid=rs2.getString("alumnimeet");

	if (meetid!=null) {
	//if (meetid!="") {
	checkid=meetid.substring(0,2);
	}
	else {
	checkid="xx";
	}

	rs1=st.executeQuery("Select * from Alumni where Email='"+Email+"' and gencode='"+gen_code+"' and gen_status < 1");
	if (rs1.next()==false) {
	st1.executeUpdate("Update Alumni set alumnimeet='"+checkid+"'+'GC' where Email='"+Email+"'");
%>
	<tr><th height=20 background="http://www.sanmacs.com/images2/red_strip.gif"><p id="ftpw" align="left" style="margin-left:5px">Dear <%=name %>,</p></th></tr>
	<tr><td height=10></td></tr>
	<tr><th><p id="ftp" align=justify>Your email id or code given by you is incorrect. Please try again and enter correct email and code.</p></th></tr>
	<tr><td height=20></td></tr>
	<tr><th><input class=a type="button" onclick="history.back();" value="Back"></th></tr>
<%
	}
	else {
	st1.executeUpdate("Update Alumni set gen_status=gen_status+1 where Email='"+Email+"'");
	session.setAttribute("emid",Email);

	if (checkid.equals("Ye")) {
	st1.executeUpdate("Update Alumni set alumnimeet='Yes' where Email='"+Email+"'");
	}
	else if (checkid.equals("No")) {
	st1.executeUpdate("Update Alumni set alumnimeet='No' where Email='"+Email+"'");
	}
	else if (checkid.equals("xx")) {
	st1.executeUpdate("Update Alumni set alumnimeet=' ' where Email='"+Email+"'");
	}
%>
	<tr><th height=20 background="http://www.sanmacs.com/images2/red_strip.gif"><p id="ftpw" align="left" style="margin-left:5px">Dear <%=name %>,</p></th></tr>
	<tr><td height=10></td></tr>
	<tr><th><p id="ftp" align=justify>Thanks for the confirmation. Now you can modify your information!</p></th></tr>
	<tr><td height=20></td></tr>
